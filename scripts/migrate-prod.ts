...
if (VERCEL_ENV === "production") {
  ...
  if (DATABASE_URL.includes("localhost") || DATABASE_URL.includes("127.0.0.1")) {
    ...
    console.error("DATABASE_URL must point to a remote database (e.g., Neon, Supabase, etc.)");
  ...
  try {
    ...
    console.error("\nTroubleshooting:");
    console.error("1. Verify DATABASE_URL is correct in Vercel environment variables");
    console.error("2. Check that your database is accessible from Vercel's IP addresses");
    console.error("3. Ensure your database allows connections from external IPs");
